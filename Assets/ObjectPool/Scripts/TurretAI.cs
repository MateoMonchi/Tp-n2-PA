...
public class TurretAI : MonoBehaviour
{
    ...
    {
        GameObject pooledBullet = null;

        if (turretType == TurretType.Catapult)
        {
            lockOnPos = go.transform;
            pooledBullet = BulletPool.SharedInstance.GetPooledCatapult();
        }
        else if (turretType == TurretType.Dual)
        {
            pooledBullet = BulletPool.SharedInstance.GetPooledMissile();
        }
        else
        {
            pooledBullet = BulletPool.SharedInstance.GetPooledGuns();
        }

        if (pooledBullet != null)
        {
            pooledBullet.transform.position = turretType == TurretType.Dual && !shootLeft ? muzzleSub.position : muzzleMain.position;
            pooledBullet.transform.rotation = turretType == TurretType.Dual && !shootLeft ? muzzleSub.rotation : muzzleMain.rotation;
            pooledBullet.SetActive(true);

            Projectile projectile = pooledBullet.GetComponent<Projectile>();
            ...
            Instantiate(muzzleEff, pooledBullet.transform.position, pooledBullet.transform.rotation);
        }
    }
}
